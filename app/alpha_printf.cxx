...
static u_char *sprintf_num(u_char *buffer, u_char *last, uint64_t ui64, u_char zero, uintptr_t hexadecimal, uintptr_t width)
{
    u_char *p, temp[ALPHA_INT64_LEN + 1];
    size_t len;
    uint32_t ui32;
    ...
    p = temp + ALPHA_INT64_LEN;

    if (hexadecimal == 0)
    {
        if (ui64 <= (uint64_t)ALPHA_MAX_UINT32_VALUE)
        {
            ui32 = (uint32_t)ui64;
            do
            {
    ...
    return Alpha_Cpy_Mem(buffer, p, len);
}